...
async fn handle_sse_event(
    config: &Config,
    event: sse::SseEvent,
    advertised_models: &mut Vec<String>,
    node_endpoints: &std::collections::HashMap<String, (String, String)>,
    semaphore: &Arc<Semaphore>,
    tx: &mpsc::UnboundedSender<String>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let text = if event.data.is_empty() { event.event_type.clone() } else { event.data };
...
async fn run_sse_connection(config: &Config, max_threads: usize) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let (stream_token, mut events) = sse::connect_downlink(config, semaphore.clone())
        .await
        .map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { e.into() })?;
    info!("SSE downlink open (token {}…)", &stream_token[..stream_token.len().min(8)]);
    ...
    for node_config in &config.nodes {
        ...
        if let Err(e) = sse::register_node(
            config,
            &stream_token,
            &node_config.alias,
            &models,
    ...
    if let Err(e) = sse::heartbeat(config, &stream_token, Some(advertised_models.as_slice())).await {
    ...
    while RUNNING.load(Ordering::SeqCst) {
        tokio::select! {
            response_json = rx.recv() => {
                ...
                match msg_type {
                    "INFERENCE_CHUNK" => {
                        ...
                        if let Err(e) = sse::inference_chunk(config, &stream_token, &request_id, index, delta, kind).await {
                            error!("SSE chunk uplink failed: {}", e);
                        }
                    }
                    "INFERENCE_RESPONSE" | "INFERENCE_ERROR" | "TTS_RESPONSE" | "TTS_ERROR" | "INTERVIEW_RESULT" => {
                        if let Err(e) = sse::inference_result(config, &stream_token, &request_id, &parsed).await {
                            error!("SSE result uplink failed: {}", e);
                        }
                    }
                    other => {
                        warn!("Unhandled SSE uplink type {}", other);
                    }
                }
            }
            event = events.recv() => {
                match event {
                    Some(event) => {
                        handle_sse_event(
                            config, event, &mut advertised_models, &node_endpoints, &semaphore, &tx,
                        ).await?;
                    }
                    None => {
            ...
            _ = heartbeat.tick() => {
                if let Err(e) = sse::heartbeat(config, &stream_token, None).await {
